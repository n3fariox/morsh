...
fn build_vendored(link_mode: LinkMode) {
    ...
    match link_mode {
        ...
        LinkMode::Static => {
            // On Windows, MSVC resolves `ghostty-vt` to `ghostty-vt.lib`, which is
            // the *DLL import library* (43 KB) rather than the real static archive
            // `ghostty-vt-static.lib` (several MB). Linking the import lib yields a
            // runtime dependency on ghostty-vt.dll. Name the archive explicitly so
            // the linker pulls in the self-contained static archive instead.
            if target.contains("windows") {
